function all_params = initProblem(varargin)
    ...
    if nargin>=7;  c_t     = varargin{7}{1} ; else ; c_t = 10000;    end            % Cost per delay
    ...
    % Bandwidths
    Bu = 20e6;      % Total Iot-HAPS Bandwidth [Hz]
    ...
    Ph      = 2;            % Total power available at UAV [W]
    ...
    Pnh_dBm      = pow2db(db2pow(N0)*Bu);           % IoT-HAPS noise power in dBm
    Pnh          = db2pow(Pnh_dBm)*(1e-3);          % IoT-HAPS noise power in W
    Pns_dBm      = pow2db(db2pow(N0)*Bh);           % HAPS-LEO noise power in dBm
    Pns          = db2pow(Pns_dBm)*(1e-3);          % HAPS-LEO noise power in W
    Png_dBm      = Pns_dBm;                         % LEO-GW noise power in dBm
    Png          = Pns;                             % LEO-GW noise power in W
    ...
    rng(2018+iMC);
        ...
        Hih_c = get_hiu(rh, ri_c, A_c, fiu, B);
        Gih   = sum(conj(Hih_c).*Hih_c,1);      % Channel Gains
        SNR_i = Pi*Gih'/Pnh;                    % SNR
